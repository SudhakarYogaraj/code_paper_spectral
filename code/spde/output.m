...
% Construction of the drift-term at the right-hand side of the original equation
    ...
    drif_aux = subs(drif_aux, epsilon, 0);
...
%% Calculation of the coefficients of the effective equation
    ...
    for i = 1:d
        for j = 1:d
            h1(i) = h1(i) + diff(phi(i),xs(j))*a(j);
        end
    end

    for i = 1:d;
        for j = 1:N-d
            h2(i) = h2(i) + diff(phi(i),ys(j))*b2(j);
        end
    end
    ...
    for i = 1:N-d
        ...
        if(abs(q)>1e-10)
            ...
            h1 = simplify(int(h1*rho,ys(i),-inf,inf));
            h2 = simplify(int(h2*rho,ys(i),-inf,inf));
        else
            g = subs(g,ys(i),0);
            h1 = subs(h1,ys(i),0);
            h2 = subs(h2,ys(i),0);
