...
double Problem::potential(vector<double> x, vector<double> y) {
    double result = 0.;
    result = (y[0]*y[0])*(-1.0/2.0)+y[1]*y[1]+(y[0]*y[0]*y[0]*y[0])*(1.0/4.0);
    return result;
}

double Problem::linearTerm(vector<double> x, vector<double> y){
    double result;
    result = pow(y[0]-y[0]*y[0]*y[0],2.0)*(-1.0/4.0)+(y[0]*y[0])*(3.0/2.0)-y[1]*y[1]+1.0/2.0;
    return result;
}

double Problem::rho(vector<double> x, vector<double> y) {
    double result = 0.;
    result = exp((y[0]*y[0])*(1.0/2.0)-y[1]*y[1]-(y[0]*y[0]*y[0]*y[0])*(1.0/4.0))*1.444736917018155E-1;
    return result;
}
...
vector<double> Problem::drif(vector<double> x, vector<double> y) {
    ...
    result[0] = y[0]-y[0]*y[0]*y[0];
    result[1] = y[1]*-2.0;
    result[2] = -y[2]*((y[0]*y[0])*3.0-1.0)+cos(x[0])*cos(y[0])*cos(y[1]);
